  ...
  class Sender
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LINE::Client::MessagingApi::Sender` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LINE::Client::MessagingApi::Sender`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@icon_url.nil? && @icon_url.to_s.length > 2000
        invalid_properties.push('invalid value for "icon_url", the character length must be smaller than or equal to 2000.')
      end
    ...
    def valid?
      ...
      return false if !@icon_url.nil? && @icon_url.to_s.length > 2000
      true
    end
    ...
    def icon_url=(icon_url)
      ...
      if icon_url.to_s.length > 2000
        fail ArgumentError, 'invalid value for "icon_url", the character length must be smaller than or equal to 2000.'
      end
