  ...
  class MembersIdsResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LINE::Client::MessagingApi::MembersIdsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LINE::Client::MessagingApi::MembersIdsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @member_ids.length > 100
        invalid_properties.push('invalid value for "member_ids", number of items must be less than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @member_ids.length > 100
      true
    end
    ...
    def member_ids=(member_ids)
      if member_ids.nil?
        fail ArgumentError, 'member_ids cannot be nil'
      end

      if member_ids.length > 100
        fail ArgumentError, 'invalid value for "member_ids", number of items must be less than or equal to 100.'
      end
